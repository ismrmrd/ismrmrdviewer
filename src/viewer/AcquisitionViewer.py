...
class AcquisitionViewer(QtWidgets.QSplitter):
    ...
    def mouse_clicked(self, index):
        ...
        y = index.column()
        print(index.column())
        #self.x, self.y = self.columnAt(QtGui.QCursor.pos())
        DeleteAction.triggered.connect(lambda: self.acquisitions.hideColumn(0))
        menu.addAction(DeleteAction)
        menu.popup(QtGui.QCursor.pos())

        SortAction = QtWidgets.QAction('Sort', self)
        #SortAction.triggered.connect(lambda: self.acquisitions.removeColumn(0))
